...
//     // Initialize Supabase client with awaited cookies for Next.js 15
//     let supabase;
//     try {
...
//       hasFile: !!submission.submission_file_url,
//       caseStudyTitle: submission.case_studies?.title,
//       clerkId: submission.clerk_id // Log the Clerk ID to verify it's being stored
//     });
...
export async function POST(request) {
  ...
  try {
    ...
    // Groq AI evaluation with error handling
    let chatCompletion, responseText;
    try {
      console.log('Initializing Groq AI...');

      const prompt = `You are an expert evaluator for business case studies. Please evaluate the following submission based on these criteria:
...
- Highlight strengths and areas for improvement
- Be constructive and encouraging
- Respond ONLY with a valid JSON object with "score" and "feedback" fields
  ...
  "feedback": "Excellent analysis of the problem with creative solutions. The implementation plan is well-structured and realistic. Consider adding more specific metrics for measuring success."
      ...
      const groqPromise = groq.chat.completions.create({
        messages: [
          {
            role: "system",
            content: "You are an expert case study evaluator. Always respond with valid JSON containing 'score' (number) and 'feedback' (string) fields."
          },
          {
            role: "user",
            content: prompt
      ...
      chatCompletion = await Promise.race([groqPromise, timeoutPromise]);
      ...
      // Check for specific error types
      if (aiError.message?.includes('API') || aiError.message?.includes('key')) {
        return new Response(
          JSON.stringify({ 
            error: "Invalid API key", 
      ...
      if (aiError.message?.includes('quota') || aiError.message?.includes('limit') || aiError.message?.includes('rate')) {
        return new Response(
          JSON.stringify({ 
            error: "API quota exceeded", 
            details: "Please check your Groq API quota and rate limits" 
          }), 
          { status: 500, headers: { 'Content-Type': 'application/json' } }
        );
      }
    ...
    try {
      ...
      console.log('JSON parsing failed, extracting manually');
      console.log('Parse error:', parseError.message);
      console.log('Raw text to parse:', responseText);
      
      // If JSON parsing fails, extract score and use text as feedback
      const scoreMatch = responseText.match(/(?:score|Score).*?(\d+)/);
      evaluation = {
        score: scoreMatch ? parseInt(scoreMatch[1]) : 75,
