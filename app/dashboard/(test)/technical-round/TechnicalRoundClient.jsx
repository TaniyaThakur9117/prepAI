...
//woking code but not taking input 
//app\dashboard\(test)\technical-round\TechnicalRoundClient.jsx
// 'use client'
// import { useEffect, useState } from "react";
// import { Sun, Moon, Play, Check, X, Code, Bug, HelpCircle, Trophy, Clock, ChevronRight, ChevronLeft, Save } from "lucide-react";
...
//   const [isRunningCode, setIsRunningCode] = useState(false);
//   const [codeOutput, setCodeOutput] = useState({});
//   const [selectedLanguages, setSelectedLanguages] = useState({}); // Track language per question
...
//   const languages = [
//     { id: 63, name: "JavaScript (Node.js)", icon: "JS" },
//     { id: 71, name: "Python 3", icon: "PY" },
//     { id: 62, name: "Java", icon: "JAVA" },
//     { id: 54, name: "C++ (GCC 9.2.0)", icon: "C++" },
//     { id: 50, name: "C (GCC 9.2.0)", icon: "C" },
//     { id: 51, name: "C# (Mono 6.6.0.161)", icon: "C#" },
//     { id: 78, name: "Kotlin", icon: "KT" },
//     { id: 68, name: "PHP", icon: "PHP" },
//     { id: 72, name: "Ruby", icon: "RB" },
//     { id: 73, name: "Rust", icon: "RS" },
...
//   // Initialize coding answers with starter code and default language
//   useEffect(() => {
//     if (codingQs.length > 0) {
//       const initialAnswers = {};
...
//           initialAnswers[q.id] = q.starter_code || '';
//         }
//         if (!selectedLanguages[q.id]) {
//           initialLanguages[q.id] = 63; // Default to JavaScript
//         }
//       });
//       if (Object.keys(initialAnswers).length > 0) {
//         setCodingAnswers(prev => ({ ...prev, ...initialAnswers }));
...
//   // Handle language selection for a question
//   const handleLanguageChange = (questionId, languageId) => {
//     setSelectedLanguages(prev => ({ ...prev, [questionId]: languageId }));
//   };
...
//     const languageId = selectedLanguages[questionId] || 63;
...
//       try {
//         const executionSuccess = result.status?.description === 'Accepted' || result.status === 'Accepted';
//         const executionScore = executionSuccess ? 25 : 20;
...
//         <div className="text-center">
//           <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500 mx-auto mb-4"></div>
//           <p className={`text-lg font-semibold ${isDark ? 'text-gray-200' : 'text-gray-700'}`}>Loading questions...</p>
//         </div>
//       </div>
//     );
//   }
...
//         <div className="max-w-4xl mx-auto">
//           <div className={`rounded-2xl shadow-sm border p-8 text-center ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
//             <Trophy className="w-20 h-20 text-yellow-500 mx-auto mb-6" />
//             <h1 className={`text-4xl font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>Technical Assessment Complete!</h1>
//             <div className="text-6xl font-bold text-blue-500 mb-4">{score.percentage}%</div>
//             <p className={`text-xl mb-6 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{score.totalScore} out of {score.maxScore} points</p>
            
//             <div className={`border rounded-xl p-4 mb-6 ${isDark ? 'bg-green-900 border-green-700' : 'bg-green-50 border-green-200'}`}>
//               <div className="flex items-center justify-center space-x-2">
//                 <Save className="w-5 h-5 text-green-600" />
...
//             <div className="grid md:grid-cols-3 gap-6 mb-8">
//               <div className={`border rounded-xl p-4 ${isDark ? 'bg-blue-900 border-blue-700' : 'bg-blue-50 border-blue-100'}`}>
//                 <HelpCircle className="w-8 h-8 text-blue-500 mx-auto mb-2" />
//                 <h3 className={`font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>MCQs</h3>
//                 <p className={isDark ? 'text-gray-300' : 'text-gray-600'}>{Object.keys(mcqAnswers).length}/{mcqs.length}</p>
//               </div>
//               <div className={`border rounded-xl p-4 ${isDark ? 'bg-orange-900 border-orange-700' : 'bg-orange-50 border-orange-100'}`}>
//                 <Bug className="w-8 h-8 text-orange-500 mx-auto mb-2" />
//                 <h3 className={`font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Debug</h3>
//                 <p className={isDark ? 'text-gray-300' : 'text-gray-600'}>{Object.keys(debugAnswers).length}/{debugQs.length}</p>
//               </div>
//               <div className={`border rounded-xl p-4 ${isDark ? 'bg-green-900 border-green-700' : 'bg-green-50 border-green-100'}`}>
//                 <Code className="w-8 h-8 text-green-500 mx-auto mb-2" />
//                 <h3 className={`font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Coding</h3>
//                 <p className={isDark ? 'text-gray-300' : 'text-gray-600'}>{Object.keys(codingAnswers).length}/{codingQs.length}</p>
//               </div>
//             </div>
...
//       <div className={`min-h-screen flex items-center justify-center ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
//         <div className={`rounded-2xl shadow-sm border p-8 text-center ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
//           <h2 className={`text-2xl font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>No Questions Available</h2>
...
//       {/* Header */}
//       <header className={`shadow-sm border-b sticky top-0 z-10 ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
//         <div className="max-w-7xl mx-auto px-6 py-4">
//           <div className="flex items-center justify-between">
//             <div className="flex items-center space-x-4">
//               <div className="bg-gradient-to-r from-blue-500 to-purple-600 p-2 rounded-xl">
...
//               </div>
//               <div>
//                 <h1 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Technical Assessment</h1>
//                 <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Section: <span className={`font-medium capitalize ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{currentSection}</span> ({currentIndex + 1}/{getSectionData().length})</p>
//               </div>
//             </div>
            
//             <div className="flex items-center space-x-6">
//               {/* Timer */}
//               <div className={`flex items-center space-x-2 px-4 py-2 rounded-xl border ${isDark ? 'bg-red-900 border-red-700' : 'bg-red-50 border-red-100'}`}>
//                 <Clock className="w-5 h-5 text-red-500" />
//                 <span className="font-mono text-lg font-bold text-red-600">
//                   {formatTime(timeLeft)}
//                 </span>
...
//               <button
//                 onClick={() => setIsDark(!isDark)}
//                 className={`p-2 rounded-xl transition-colors duration-200 ${isDark ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-100 hover:bg-gray-200'}`}
//               >
...
//               <h3 className={`text-lg font-semibold mb-6 ${isDark ? 'text-white' : 'text-gray-900'}`}>Sections</h3>
...
//               {/* Progress Overview */}
//               <div className={`border-t pt-6 mb-6 ${isDark ? 'border-gray-700' : 'border-gray-100'}`}>
//                 <h4 className={`text-sm font-semibold mb-4 uppercase tracking-wide ${isDark ? 'text-gray-300' : 'text-gray-900'}`}>Progress</h4>
//                 <div className="space-y-3">
//                   {mcqs.length > 0 && (
//                     <div className="flex justify-between items-center">
//                       <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>MCQs:</span>
//                       <span className={`text-sm font-medium ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{Object.keys(mcqAnswers).length}/{mcqs.length}</span>
//                     </div>
//                   )}
//                   {debugQs.length > 0 && (
//                     <div className="flex justify-between items-center">
//                       <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Debug:</span>
//                       <span className={`text-sm font-medium ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{Object.keys(debugAnswers).length}/{debugQs.length}</span>
//                     </div>
//                   )}
//                   {codingQs.length > 0 && (
//                     <div className="flex justify-between items-center">
//                       <span className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Coding:</span>
//                       <span className={`text-sm font-medium ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>{Object.keys(codingAnswers).length}/{codingQs.length}</span>
//                     </div>
//                   )}
//                 </div>
//               </div>
...
//                       </div>
//                       <div>
//                         <h2 className={`text-2xl font-bold capitalize ${isDark ? 'text-white' : 'text-gray-900'}`}>{currentSection} Question {currentIndex + 1}</h2>
//                         <p className={isDark ? 'text-gray-400' : 'text-gray-500'}>Practice technical interview questions and get instant feedback.</p>
//                       </div>
//                     </div>
//                     <div className={`px-4 py-2 rounded-full text-sm font-medium ${
//                       currentSection === 'mcq' ? 'bg-blue-100 text-blue-800' :
...
//                   {/* Question Content */}
//                   <div className="mb-8">
//                     <div className={`rounded-xl p-6 mb-6 ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
//                       <h3 className={`font-semibold mb-3 ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>Question:</h3>
//                       <p className={`text-lg leading-relaxed ${isDark ? 'text-gray-300' : 'text-gray-800'}`}>{currentQuestion.question}</p>
//                     </div>
...
//                       <div className="space-y-6">
//                         <div>
//                           <h4 className={`font-semibold mb-3 ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>Code to Debug:</h4>
//                           <div className={`rounded-xl p-6 overflow-x-auto ${isDark ? 'bg-gray-900' : 'bg-gray-900'}`}>
//                             <pre className="text-green-400 text-sm font-mono">
//                               <code>{currentQuestion.code}</code>
//                             </pre>
//                           </div>
...
//                         <div>
//                           <h4 className={`font-semibold mb-3 ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>Your Analysis & Fix:</h4>
//                           <textarea
//                             value={debugAnswers[currentQuestion.id] || ''}
//                             onChange={(e) => handleDebugAnswer(currentQuestion.id, e.target.value)}
//                             placeholder="Describe the bug and provide your fixed code..."
//                             className={`w-full h-48 p-4 rounded-xl border font-mono text-sm resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
//                               isDark ? 'bg-gray-700 border-gray-600 text-gray-200 placeholder-gray-400' : 'border-gray-200 text-gray-900'
//                             }`}
//                           />
//                         </div>
//                       </div>
...
//                         {/* Language Selector */}
//                         <div>
//                           <h4 className={`font-semibold mb-3 ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>Select Programming Language:</h4>
//                           <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
//                             {languages.map((lang) => (
//                               <button
//                                 key={lang.id}
//                                 onClick={() => handleLanguageChange(currentQuestion.id, lang.id)}
//                                 className={`p-3 rounded-xl border-2 transition-all duration-200 ${
//                                   selectedLanguages[currentQuestion.id] === lang.id
...
//                                 }`}
//                               >
//                                 <div className="text-xs font-bold mb-1">{lang.icon}</div>
//                                 <div className="text-xs truncate">{lang.name.split('(')[0]}</div>
//                               </button>
//                             ))}
//                           </div>
//                         </div>
...
//                         {currentQuestion.starter_code && (
//                           <div>
//                             <h4 className={`font-semibold mb-3 ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>Starter Code:</h4>
//                             <div className={`rounded-xl p-6 overflow-x-auto ${isDark ? 'bg-gray-900' : 'bg-gray-900'}`}>
//                               <pre className="text-green-400 text-sm font-mono">
//                                 <code>{currentQuestion.starter_code}</code>
//                               </pre>
...
//                         <div>
//                           <div className="flex items-center justify-between mb-3">
//                             <h4 className={`font-semibold ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>Your Solution:</h4>
//                             <button
//                               onClick={() => runCode(codingAnswers[currentQuestion.id], currentQuestion.id)}
//                               disabled={isRunningCode || !codingAnswers[currentQuestion.id]}
//                               className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 text-white rounded-xl font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
...
//                             onChange={(e) => handleCodingAnswer(currentQuestion.id, e.target.value)}
//                             placeholder="Write your solution here..."
//                             className={`w-full h-64 p-4 rounded-xl border font-mono text-sm resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
//                               isDark ? 'bg-gray-700 border-gray-600 text-gray-200 placeholder-gray-400' : 'border-gray-200 text-gray-900'
//                             }`}
//                           />
//                         </div>
...
//                         {codeOutput[currentQuestion.id] && (
//                           <div>
//                             <h4 className={`font-semibold mb-3 ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>Output:</h4>
//                             <div className={`rounded-xl p-6 ${isDark ? 'bg-gray-900' : 'bg-gray-900'}`}>
//                               <div className="flex items-center space-x-2 mb-4">
//                                 <span className={`px-3 py-1 rounded-full text-xs font-medium ${
//                                   codeOutput[currentQuestion.id].status === 'Accepted' ? 
//                                   'bg-green-100 text-green-800' :
...
//                   {/* Navigation */}
//                   <div className={`flex items-center justify-between pt-8 border-t ${isDark ? 'border-gray-700' : 'border-gray-100'}`}>
//                     <button
//                       onClick={prevQuestion}
//                       disabled={currentSection === 'mcq' && currentIndex === 0}
//                       className={`flex items-center space-x-2 px-6 py-3 rounded-xl font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 ${
...
export default function TechnicalRoundClient() {
  ...
  const [codeOutput, setCodeOutput] = useState({});
  const [selectedLanguages, setSelectedLanguages] = useState({}); // Track language per question
  const [codeInputs, setCodeInputs] = useState({}); // Track stdin input per question
  ...
  const handleInputChange = (questionId, input) => {
    setCodeInputs(prev => ({ ...prev, [questionId]: input }));
  };
  ...
  const runCode = async (code, questionId) => {
    ...
    const languageId = selectedLanguages[questionId] || 63;
    const stdin = codeInputs[questionId] || ''; // Get input for this question
                      ...
                      <div className="space-y-6">
                        ...
                        <div>
                          <h4 className={`font-semibold mb-3 ${isDark ? 'text-gray-200' : 'text-gray-900'}`}>
                            Input (stdin) - <span className="text-sm font-normal text-gray-500">Optional: Enter input data for your program</span>
                          </h4>
                          <textarea
                            value={codeInputs[currentQuestion.id] || ''}
                            onChange={(e) => handleInputChange(currentQuestion.id, e.target.value)}
                            placeholder="Enter input data here (each line will be passed to your program)&#10;Example for Python:&#10;5&#10;10&#10;15"
                            rows={4}
                            className={`w-full p-4 rounded-xl border font-mono text-sm resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                              isDark ? 'bg-gray-700 border-gray-600 text-gray-200 placeholder-gray-400' : 'border-gray-200 text-gray-900 placeholder-gray-500'
                            }`}
                          />
                          <div className={`mt-2 text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                            💡 Tip: Use this field to provide input that your program reads using scanf(), input(), cin, etc.
                          </div>
                        </div>
